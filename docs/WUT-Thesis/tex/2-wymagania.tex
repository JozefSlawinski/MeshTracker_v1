...
Tak zdefiniowany problem i charakterystyka użytkownika prowadzą do następujących wymagań funkcjonalnych, które muszą być spełnione przez system:
System musi odbierać dane o pozycji GPS węzła mobilnego i wyświetlać pozycję wszystkich
aktywnych węzłów na mapie w czasie rzeczywistym, przechowując historię pozycji z bieżącej sesji.
Użytkownik musi mieć możliwość zdefiniowania jednej lub więcej stref bezpieczeństwa w postaci
wielokątów na mapie. System musi wykrywać przekroczenie granicy strefy przez węzeł mobilny
i powiadamiać użytkownika - również gdy aplikacja działa w tle lub telefon ma wygaszony ekran
- przy czym zdefiniowane strefy muszą być przechowywane między sesjami aplikacji.
Dodatkowo system powinien różnicować częstotliwość wysyłania pozycji w zależności od stanu ruchu
węzła, co pozwoli na oszczędzanie pasma i baterii węzłów, oraz umożliwiać przypisanie nazwy
i ikony do każdego węzła.
...
Wymaganie to wynika z analizy szczególnego scenariusza użytkowania: Mały Gończy Gaskoński jest rasą
gończą hodowaną do długodystansowych pogoni, zdolną do utrzymania biegu z prędkością
...
pośredniczących - co jest niemożliwe w architekturze punkt-punkt stosowanej przez Dogtrę
i Dogtrace.
